...
import { useAppStore, type ViewKey, canAccess } from "@/lib/store";
...
interface NavItem {
  ...
  roles: ("SUPER_ADMIN" | "COACH" | "VIEWER")[];
}

const NAV: NavItem[] = [
  { key: "dashboard", label: "Dashboard", icon: LayoutDashboard, roles: ["SUPER_ADMIN", "COACH", "VIEWER"] },
  { key: "timer", label: "Timing Console", icon: Timer, roles: ["SUPER_ADMIN", "COACH"] },
  { key: "swimmers", label: "Swimmers", icon: Users, roles: ["SUPER_ADMIN", "COACH"] },
  { key: "groups", label: "Groups / Heats", icon: Grid3x3, roles: ["SUPER_ADMIN", "COACH"] },
  { key: "history", label: "Session History", icon: History, roles: ["SUPER_ADMIN", "COACH", "VIEWER"] },
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const current = items.find((n) => n.key === view) || items[0];

  // Mobile: hide bottom nav on timer screen for max space? Keep it but make it compact.
  return (
    <div className="min-h-screen flex flex-col bg-background">
      {/* Header (sticky top) */}
      <header className="sticky top-0 z-40 h-14 border-b bg-sidebar text-sidebar-foreground flex items-center px-3 sm:px-4 gap-3">
        <button
          className="lg:hidden p-2 -ml-1 rounded-md hover:bg-sidebar-accent"
        ...
        <div className="ml-auto flex items-center gap-2 sm:gap-3">
          ...
          <Badge
            variant="secondary"
            className="hidden sm:inline-flex bg-aqua/20 text-aqua border-aqua/30"
          >
            {current?.label}
          </Badge>
          <Button
      ...
      <div className="flex flex-1">
        ...
        <aside className="hidden lg:flex w-60 flex-col border-r bg-sidebar">
          ...
          <div className="p-3 border-t border-sidebar-border text-[10px] text-sidebar-foreground/50">
            LanePulse Pro v1.0 • 12-Lane Pool
          </div>
        </aside>
        ...
        {sidebarOpen && (
          <div className="lg:hidden fixed inset-0 z-50 flex">
            ...
            />
            <aside className="relative w-64 bg-sidebar text-sidebar-foreground flex flex-col animate-in slide-in-from-left">
      ...
      {/* Mobile bottom nav */}
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-sidebar border-t border-sidebar-border flex items-stretch h-14 safe-bottom">
